...
function isTimestampWindowError(retCode: unknown, retMsg: unknown): boolean {
  ...
  return code === 10002
    || code === -1
    || msg.includes("time exceeds the time window")
    || msg.includes("server timestamp")
    || msg.includes("recv_window")
...
export class BybitRealRestClient {
    ...
    while (true) {
      ...
      try {
        ...
        if (isTimestampWindowError(parsed?.retCode, parsed?.retMsg) && !retried10002) {
          ...
          return this.requestInternal(method, endpoint, query, body, opts, retried, true);
        }
        ...
        if (!res.ok || !parsed || (parsed.retCode !== 0 && !shouldIgnoreRetCode)) {
          const err: any = new Error(`Bybit real REST error: ${endpoint}`);
  ...
  async getClosedPnlLinear(params: { symbol?: string; startTime?: number; endTime?: number; limit?: number; cursor?: string } = {}): Promise<{ list: ClosedPnlEntry[]; nextPageCursor?: string }> {
    const result = await this.request<{ list?: ClosedPnlEntry[]; nextPageCursor?: string }>("GET", "/v5/position/closed-pnl", {
      category: "linear",
      ...params,
    });
    return {
  ...
  async getTransactionLogLinear(params: { currency?: string; startTime?: number; endTime?: number; limit?: number; cursor?: string } = {}): Promise<{ list: TransactionLogEntry[]; nextPageCursor?: string }> {
    const result = await this.request<{ list?: TransactionLogEntry[]; nextPageCursor?: string }>("GET", "/v5/account/transaction-log", {
      accountType: "UNIFIED",
      category: "linear",
      ...params,
    });
